# taken from https://gist.github.com/rtavenar/a4fb580ae235cc61ce8cf07878810567 on 07.09.2021
# no info about copyright in the original source!!!
# 
...
if __name__ == "__main__":
    ...
    from dtaidistance import dtw_missing as dtwm
    
    n1, n2, p = 5, 10, 1000
    X1 = numpy.random.randn(n1, p)
    X2 = numpy.random.randn(n2, p)
    
    # dist_fun = euclidean
    dist_fun = lambda o, oo: dtwm.warping_paths(None, o, oo)[0]

    mat_cdist = cdist_generic(dist_fun=dist_fun, dataset1=X1, dataset2=X2,
                              compute_diagonal=False, n_jobs=None, verbose=1)
    assert(mat_cdist.shape == (n1, n2))

    mat_cdist = cdist_generic(dist_fun=dist_fun, dataset1=X1, dataset2=X2,
                              compute_diagonal=False, n_jobs=-1, verbose=1)
    assert(mat_cdist.shape == (n1, n2))
    # NB: this should be useful in cases where each distance computation is
    # long and parallelizing is worth the overhead.
    # Euclidean distance is not the ideal example for that...